...
public class JwtAuthenticationFilter extends BasicAuthenticationFilter {
    ...
    private final JwtTokenManger jwtTokenManger; // 오타가 아니라면 Manger 그대로 유지 (일반적으론 Manager)
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            // 3. "Bearer " 뒤의 순수한 토큰 문자열만 추출 ("Bearer "은 7글자이므로 하드코딩이 안전합니다)
            ...
            // 고도화: 인증 실패 시 다음 필터로 넘기지 않고, 여기서 즉시 401 에러를 반환합니다.
            sendErrorResponse(response, "토큰이 만료되었거나 유효하지 않습니다.");
        }
    }

    /**
     * 인증 실패 시 클라이언트에게 JSON 형태의 에러 메시지를 내려주는 헬퍼 메서드
     */
